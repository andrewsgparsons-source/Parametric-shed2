...
function buildApex(state, ctx) {
  ...
  // Truss layout (constant orientation):
  // A = span axis (shed WIDTH), B = ridge/run axis (shed DEPTH)
  const A_mm = roofW_mm;
  const B_mm = roofD_mm;

  // Rule A: ridge ALWAYS along shed DEPTH (world Z).
  const ridgeAlongWorldX = false;
